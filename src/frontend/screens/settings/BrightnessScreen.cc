...
BrightnessScreen::BrightnessScreen(BackendInterface *backend, ScreenManager *screenManager, Settings *settings)
...
void BrightnessScreen::updateCheckBoxes() {
    DisplayInterface::BrightnessLevel currentLevel = DisplayInterface::BrightnessLevel::Low;
    if (this->settings()->displayBrightness() == BRIGHTNESS_MEDIUM) {
        currentLevel = DisplayInterface::BrightnessLevel::Medium;
    }
    else if (this->settings()->displayBrightness() >= BRIGHTNESS_HIGH) {
        currentLevel = DisplayInterface::BrightnessLevel::High;
    }
    for (OptionInfo *optionInfo : _optionInfos) {
        bool checked = false;
        switch (optionInfo->row) {
            case 0:   // dark
                checked = (currentLevel == DisplayInterface::BrightnessLevel::Low);
                break;
            case 1:   // medium
                checked = (currentLevel == DisplayInterface::BrightnessLevel::Medium);
                break;
            case 2:   // light
                checked = (currentLevel == DisplayInterface::BrightnessLevel::High);
                break;
            default:
                break;
        }
...
void BrightnessScreen::optionClicked(int row) {
    DisplayInterface::BrightnessLevel newLevel;
    switch (row) {
        case 1:
            newLevel = DisplayInterface::BrightnessLevel::Medium;
            this->settings()->setDisplayBrightness(BRIGHTNESS_MEDIUM);
            break;
        case 2:
            newLevel = DisplayInterface::BrightnessLevel::High;
            this->settings()->setDisplayBrightness(BRIGHTNESS_HIGH);
            break;
        default:
            newLevel = DisplayInterface::BrightnessLevel::Low;
            this->settings()->setDisplayBrightness(BRIGHTNESS_LOW);
            break;
    }
    this->backend()->display()->setBrightness(newLevel);
    updateCheckBoxes();
}
