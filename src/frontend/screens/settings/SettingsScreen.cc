...
SettingsScreen::SettingsScreen(BackendInterface *backend, ScreenManager *screenManager, Settings *settings)
    ...
    // add the buttons
    addButton(&FontAwesomeRegular24, SYMBOL_SUN, "Brightness", ScreenManager::ScreenId::BrightnessScreen);
    addButton(&FontAwesomeSolid24, SYMBOL_BED, "Wake up", ScreenManager::ScreenId::WakeUpModeScreen);
    addButton(&FontAwesomeSolid24, SYMBOL_WALKING, "Activity goal", ScreenManager::ScreenId::StepsGoalScreen);
    addButton(&FontAwesomeSolid24, SYMBOL_INFO, "System info", ScreenManager::ScreenId::SystemInfoScreen);
    addButton(&FontAwesomeRegular24, SYMBOL_SAVE_DISK, "Validate firmware", ScreenManager::ScreenId::FirmwareValidationScreen);
}
...
void SettingsScreen::addButton(const lv_font_t *iconFont, const char *icon, const char *text, ScreenManager::ScreenId destScreen) {
...
void SettingsScreen::buttonClicked(int row) {
    ...
    for (ButtonInfo *buttonInfo : _buttons) {
        if (((buttonInfo->page + 1) == currentPage()) && (buttonInfo->row == row)) {
            this->screenManager()->activateScreen(buttonInfo->destScreen, ScreenManager::TransitionEffect::MoveLeft);
            break;
        }
    }
}
