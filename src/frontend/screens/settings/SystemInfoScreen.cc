...
SystemInfoScreen::SystemInfoScreen(BackendInterface *backend, ScreenManager *screenManager, Settings *settings)
...
void SystemInfoScreen::handleCurrentPageChange() {
    // create title label
    lv_obj_t *titleLabel = createTitleLabel("TeaTime");
...
void SystemInfoScreen::createSystemInfoPageWidgets() {
    ...
    switch (this->backend()->motion()->deviceType()) {
        case MotionInterface::DeviceType::BMA421:
            addValue(motionDeviceRow, "BMA421");
            break;
        case MotionInterface::DeviceType::BMA425:
    ...
    addValue(2, this->backend()->bluetooth()->macAddress(), true);
...
void SystemInfoScreen::refreshSystemInfoPageWidgets() {
    ...
    uint32_t uptimeSeconds = this->backend()->dateTime()->uptimeSeconds();
