...
StepsGoalScreen::StepsGoalScreen(BackendInterface *backend, ScreenManager *screenManager, Settings *settings)
...
void StepsGoalScreen::updateStepsLabel() {
    lv_label_set_text_fmt(_stepsLabel, "%d steps", this->settings()->dailyStepsGoal());
...
void StepsGoalScreen::buttonCallback(int increment, lv_obj_t *object, lv_event_t event) {
    if (event == LV_EVENT_CLICKED) {
        ...
        int newSteps = static_cast<int>(screen->settings()->dailyStepsGoal()) + increment;
        if (newSteps < MIN_STEPS_GOAL)
                newSteps = MIN_STEPS_GOAL;
        if (newSteps > MAX_STEPS_GOAL)
                newSteps = MAX_STEPS_GOAL;
        screen->settings()->setDailyStepsGoal(static_cast<uint32_t>(newSteps));
