...
#include "frontend/fonts/font_symbols_32.h"
#include "frontend/fonts/FontAwesomeBrands24.h"
#include "frontend/fonts/FontAwesomeSolid24.h"
#include "frontend/fonts/font_dvs_digits_24.h"
#include "frontend/fonts/font_dvs_digits_20.h"
#include "frontend/fonts/font_dvs_ascii_14.h"
...
InfographWatchface::InfographWatchface(BackendInterface *backend, ScreenManager *screenManager, Settings *settings)
        : Watchface(backend, screenManager, settings) {
    // initialize
    _prevSteps = this->backend()->motion()->steps();
    _prevStepsGoal = this->settings()->dailyStepsGoal();
    ...
    lv_label_set_text_fmt(_dayOfMonthLabel, "%d", static_cast<int>(day()));
...
void InfographWatchface::timeChanged(bool hourChanged, bool minuteChanged, bool secondChanged) {
    (void)secondChanged;
    ...
    // update steps arc
    uint32_t steps = this->backend()->motion()->steps();
    uint32_t stepsGoal = this->settings()->dailyStepsGoal();
...
uint16_t InfographWatchface::stepsEndAngle(uint32_t steps) {
    ...
    double g = static_cast<double>(this->settings()->dailyStepsGoal());
...
const char *InfographWatchface::dayOfWeekText() {
    char const *weekDays[] = { "SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT" };
    return weekDays[static_cast<uint8_t>(dayOfWeek())];
}
