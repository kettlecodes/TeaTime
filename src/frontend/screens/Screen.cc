...
#include "frontend/fonts/font_dvs_ascii_12.h"
#include "frontend/fonts/font_symbols_14.h"
...
Screen::Screen(BackendInterface *backend, ScreenManager *screenManager, Settings *settings) {
    // copy the references
    _backend = backend;
...
bool Screen::handleSwipeLeft() {
    return false;
}


bool Screen::handleSwipeRight() {
    return false;
}


bool Screen::handleSwipeUp() {
    // swiping up is automatically handled in multi-page screens
...
bool Screen::handleSwipeDown() {
    // swiping down is automatically handled in multi-page screens
    if (isMultiPageScreen()) {
        if (_currentPage > 1) {
            setCurrentPage(_currentPage - 1);
            return true;
